...
- **Tidal support (v2):** The most viable alternative premium service after Apple Music — Spotify deprecated the relevant APIs in Feb 2026, Amazon Music is closed-beta, Tidal is open. Their developer platform uses OAuth 2.1, exposes catalog metadata, and includes a Player SDK for in-browser playback — same conceptual shape as MusicKit JS. User base also skews toward music enthusiasts, matching the target persona. Planned as a parallel premium tier for users who pay for a streaming service but not Apple Music.
...
- **Tidal integration** — out of scope for v1. Viable for v2 (see §3); deferring keeps v1 focused on a single streaming service surface.
